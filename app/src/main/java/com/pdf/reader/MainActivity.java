...
public class MainActivity extends AppCompatActivity {
    ...
    private final ActivityResultLauncher<String[]> requestPermissionLauncher =
            registerForActivityResult(new ActivityResultContracts.RequestMultiplePermissions(), result -> {
                // 用户交互完成后再次尝试连接（无论成败由 WifiConnector 回调告知）
                startConnect();
            });
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        wifiConnector = new WifiConnector(this, new WifiConnector.Callback() {
            ...
            public void onConnecting() {
                runOnUiThread(() -> tvStatus.setText("状态: 正在连接…"));
            }

            @Override
            public void onConnected() {
                runOnUiThread(() -> tvStatus.setText("状态: 已连接到打印机热点"));
            }

            @Override
            public void onDisconnected() {
                runOnUiThread(() -> tvStatus.setText("状态: 已断开连接"));
            }

            @Override
            public void onFailed(String reason) {
                runOnUiThread(() -> tvStatus.setText("状态: 连接失败 - " + reason));
            }
        });
